...
exports.postSubmission = async (req, res, next) => {
    try {
        ...
        if (isInputInvalid(userName) || isInputInvalid(languageId) ||
            isInputInvalid(sourceCode)) {
            return res
                .status(400)
                .json('Please fill all the input fields!');
        }
        const options = {
            method: 'POST',
        ...
        const submissionDetails = {
            ...
            stdIn,
            sourceCode: sourceCode.length > 100 ? sourceCode.substr(0, 100) : sourceCode,
            token: data.token,
            stdOut: data.stdout
        }
