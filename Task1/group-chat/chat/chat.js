...
async function fetchChats(groupId, e) {
    try {
        console.log(e.target.parentNode.parentNode.id, e.target.parentNode.parentNode);
        ...
        document.getElementById('grpName').textContent = e.target.parentNode.parentNode.id;
        chatBox.innerHTML = '';
        chats.forEach(chat => {
