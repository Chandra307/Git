...
document.querySelector('#report').onclick = async (e) => {
    try {
        ...
        if (prevElement.id === 'month') {
            ...
            const { data: { expenses, amount } } = await axios.get(`http://localhost:3000/user/monthlyReport?month=${month}`, { headers: { "Authorization": token } });
            ...
            const { data: { expenses, amount } } = await axios.get(`http://localhost:3000/user/dailyReport?date=${date}`, { headers: { "Authorization": token } });
            ...
            const { data: { expenses, amount } } = await axios.get(`http://localhost:3000/user/weeklyReport?start=${start}&end=${end}`, { headers: { "Authorization": token } });
            console.log(expenses, amount);
            document.getElementById('daily').innerHTML = `<tr><th>Date</th><th>Description</th>
            <th>Category</th><th>Expense (in ₹)</th></tr>`;
            expenses.forEach(expense => {
...
document.getElementById('year').oninput = async (e) => {
    ...
    try {
        ...
        const { data: { expenses, amount } } = await axios.get(`http://localhost:3000/user/annualReport?year=${year}`, { headers: { "Authorization": token } });
